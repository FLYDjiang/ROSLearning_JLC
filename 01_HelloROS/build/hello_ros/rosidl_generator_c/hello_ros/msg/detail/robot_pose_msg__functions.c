// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from hello_ros:msg/RobotPoseMsg.idl
// generated code does not contain a copyright notice
#include "hello_ros/msg/detail/robot_pose_msg__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `angles`
#include "rosidl_runtime_c/primitives_sequence_functions.h"

bool
hello_ros__msg__RobotPoseMsg__init(hello_ros__msg__RobotPoseMsg * msg)
{
  if (!msg) {
    return false;
  }
  // x
  // y
  // z
  // angles
  if (!rosidl_runtime_c__float__Sequence__init(&msg->angles, 0)) {
    hello_ros__msg__RobotPoseMsg__fini(msg);
    return false;
  }
  return true;
}

void
hello_ros__msg__RobotPoseMsg__fini(hello_ros__msg__RobotPoseMsg * msg)
{
  if (!msg) {
    return;
  }
  // x
  // y
  // z
  // angles
  rosidl_runtime_c__float__Sequence__fini(&msg->angles);
}

bool
hello_ros__msg__RobotPoseMsg__are_equal(const hello_ros__msg__RobotPoseMsg * lhs, const hello_ros__msg__RobotPoseMsg * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // x
  if (lhs->x != rhs->x) {
    return false;
  }
  // y
  if (lhs->y != rhs->y) {
    return false;
  }
  // z
  if (lhs->z != rhs->z) {
    return false;
  }
  // angles
  if (!rosidl_runtime_c__float__Sequence__are_equal(
      &(lhs->angles), &(rhs->angles)))
  {
    return false;
  }
  return true;
}

bool
hello_ros__msg__RobotPoseMsg__copy(
  const hello_ros__msg__RobotPoseMsg * input,
  hello_ros__msg__RobotPoseMsg * output)
{
  if (!input || !output) {
    return false;
  }
  // x
  output->x = input->x;
  // y
  output->y = input->y;
  // z
  output->z = input->z;
  // angles
  if (!rosidl_runtime_c__float__Sequence__copy(
      &(input->angles), &(output->angles)))
  {
    return false;
  }
  return true;
}

hello_ros__msg__RobotPoseMsg *
hello_ros__msg__RobotPoseMsg__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hello_ros__msg__RobotPoseMsg * msg = (hello_ros__msg__RobotPoseMsg *)allocator.allocate(sizeof(hello_ros__msg__RobotPoseMsg), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(hello_ros__msg__RobotPoseMsg));
  bool success = hello_ros__msg__RobotPoseMsg__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
hello_ros__msg__RobotPoseMsg__destroy(hello_ros__msg__RobotPoseMsg * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    hello_ros__msg__RobotPoseMsg__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
hello_ros__msg__RobotPoseMsg__Sequence__init(hello_ros__msg__RobotPoseMsg__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hello_ros__msg__RobotPoseMsg * data = NULL;

  if (size) {
    data = (hello_ros__msg__RobotPoseMsg *)allocator.zero_allocate(size, sizeof(hello_ros__msg__RobotPoseMsg), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = hello_ros__msg__RobotPoseMsg__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        hello_ros__msg__RobotPoseMsg__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
hello_ros__msg__RobotPoseMsg__Sequence__fini(hello_ros__msg__RobotPoseMsg__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      hello_ros__msg__RobotPoseMsg__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

hello_ros__msg__RobotPoseMsg__Sequence *
hello_ros__msg__RobotPoseMsg__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hello_ros__msg__RobotPoseMsg__Sequence * array = (hello_ros__msg__RobotPoseMsg__Sequence *)allocator.allocate(sizeof(hello_ros__msg__RobotPoseMsg__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = hello_ros__msg__RobotPoseMsg__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
hello_ros__msg__RobotPoseMsg__Sequence__destroy(hello_ros__msg__RobotPoseMsg__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    hello_ros__msg__RobotPoseMsg__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
hello_ros__msg__RobotPoseMsg__Sequence__are_equal(const hello_ros__msg__RobotPoseMsg__Sequence * lhs, const hello_ros__msg__RobotPoseMsg__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!hello_ros__msg__RobotPoseMsg__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
hello_ros__msg__RobotPoseMsg__Sequence__copy(
  const hello_ros__msg__RobotPoseMsg__Sequence * input,
  hello_ros__msg__RobotPoseMsg__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(hello_ros__msg__RobotPoseMsg);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    hello_ros__msg__RobotPoseMsg * data =
      (hello_ros__msg__RobotPoseMsg *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!hello_ros__msg__RobotPoseMsg__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          hello_ros__msg__RobotPoseMsg__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!hello_ros__msg__RobotPoseMsg__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
